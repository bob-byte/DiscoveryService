...
namespace LUC.DiscoveryService.Ssl
{
    ...
    public class SslServer : IDisposable
    {
        ...
        public async Task<SslSession> SessionWithNewDataAsync()
        {
            SslSession sessionWithData = null;

            while (sessionWithData == null)
            {
                foreach (var sslSession in Sessions.Values)
                {
                    if (sslSession.Socket?.Available > 0)
                    {
                        sessionWithData = sslSession;
                        break;
                    }
                }
