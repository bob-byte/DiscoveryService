...
namespace LightClientLibrary
{
    ...
    public class LightClient
    {
        ...
        /// </summary>
        private const Int32 FILE_UPLOAD_CHUNK_SIZE = 2000000;
        ...
        private static String CalculateMd5Hash( Byte[] filename )
        {
        ...
        private static IEnumerable<Byte[]> IterateFileChunksWithoutFileBlocking( String filePath, Int32 offset )
        {
        ...
        private static Boolean ResponseIfChangedWhileUploadFile( String fullPath, DateTime originalModifiedDateTime )
        {
            DateTime currentModifiedDateTime = /*DateTimeExtensions.LastWriteTimeUtcWithCorrectOffset*/File.GetLastWriteTimeUtc( fullPath );

            return originalModifiedDateTime == currentModifiedDateTime;
        }
        ...
        public async Task<HttpResponseMessage> Upload( String host, String token, String userId, String bucketId, String fullPath, String filePrefix, String lastSeenVersion = "" )
        {
            ...
            String requestUri = Combine( host, "riak", "upload", bucketId );
            ...
            String version =
                ( lastSeenVersion == "" ?
                    ...
                    IncrementVersion( userId, timeStamp, lastSeenVersion ) ) ?? IncrementVersion( userId, timeStamp );
        ...
        private static void AddContentRange( HttpContent content, ChunkUploadState uploadState,
            FileInfo fileInfo )
        {
        ...
        private static void AddETags( MultipartFormDataContent content, List<String> md5S )
        {
        ...
        {
            Int32 chunkSize = fileStream.Length < FILE_UPLOAD_CHUNK_SIZE ? (Int32)fileStream.Length : FILE_UPLOAD_CHUNK_SIZE;
                                                                                       ...
                                                                                       Dictionary<String, String> uploadParams )
        {
            ...
            try
            {
                foreach ( Byte[] currentPlainBytes in IterateFileChunksWithoutFileBlocking( fullPath, offset: 0 ) )
                {
                    String currentLocalMd5 = CalculateMd5Hash( currentPlainBytes );
                    ...
                    if ( uploadParams.ContainsKey( "guid" ) && !uploadState.IsLastChunk )
                    {
                        ...
                        if ( response.Content != null )
                        {
                            ...
                            responseGetGuid.IsSuccess = response.IsSuccessStatusCode;
                        }
                        else
                        {
                            return response;
                        ...
                        if ( responseGetGuid == null || (responseGetGuid != null && !responseGetGuid.IsSuccess) )
                        {
                            return response;
                        }
                    ...
                    if ( response.Content != null )
                    {
                        ...
                        responseGet.IsSuccess = response.IsSuccessStatusCode;
                    }
                    else
                    {
                        return response;
                    ...
                    if ( responseGet != null )
                    {
                        Log.Information( $"Response {responseGet.Guid} is received for first request" );
                        ...
                        Log.Information(
                            $"Response {responseGet.UploadId} is received for part number = {uploadState.PartNumber}" );
                    }
                    ...
                    Boolean responseIfChanged = ResponseIfChangedWhileUploadFile( fullPath, File.GetLastWriteTimeUtc( fullPath ) );
                    if ( responseIfChanged )
                    {
                        continue;
                    }
                    ...
                    var errorResponse = new HttpResponseMessage
                    {
                        ...
                        ReasonPhrase = $"Upload is stopped. File {fullPath} was changed just during uploading process."
                };
            ...
            catch ( FileNotFoundException )
            {
                var errorResponse = new HttpResponseMessage
                {
                    ...
                    ReasonPhrase = $"File {fullPath} not found during uploading process."
                };
                return errorResponse;
            }
