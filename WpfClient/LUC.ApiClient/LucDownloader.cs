...
{
    class LucDownloader : SyncServicesProvider
    {
        /// <summary>
        ...
        public LucDownloader( ApiClient apiClient ) 
        ...
        )
        {
            ...
            await DownloadFileProcessAsync( downloadingFileInfo ).ConfigureAwait( continueOnCapturedContext: false );
        }
        ...
        internal void DownloadNecessaryChunksFromServer( IEnumerable<ChunkRange> undownloadedChunks, DownloadingFileInfo downloadingFileInfo, String fullFileName )
        {
            ...
            using ( FileStream fileStream = File.OpenWrite( fullFileName ) )
            {
                ...
                foreach ( ChunkRange chunkRange in undownloadedChunks.OrderBy( c => c.Start ) )
                {
        ...
        private async Task DownloadFileProcessAsync( DownloadingFileInfo downloadingFileInfo )
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    await m_dsDownloader.DownloadFileAsync( downloadingFileInfo, m_fileChangesQueue, downloadedAnyChunk ).ConfigureAwait( continueOnCapturedContext: false );
                }
                ...
                {
                    Boolean isDownloadedAnyChunk = downloadedAnyChunk.WaitOne( TimeSpan.Zero );

                    if ( isDownloadedAnyChunk && File.Exists( ex.TempFullFileName ) )//user can move or delete file
                    {
                ...
                {
                    DeleteTempFile( downloadingFileInfo );
                    throw;
                }
                ...
                LoggingService.LogInfo( logRecord: $"Download process time: {endTimeOfDownload - startTimeOfDownload} for file with size {downloadingFileInfo.ByteCount / ( 1000.0 * 1000.0 )} MB" );

                await HandleDownloadedFileAsync( downloadingFileInfo ).ConfigureAwait( false );
            }
            catch ( Exception ex )
            {
        ...
        private async Task HandleDownloadedFileAsync(
            DownloadingFileInfo downloadingFileInfo
        )
        {
            do
            {
                await Task.Delay( TimeSpan.FromSeconds( value: 0.5 ) );
            }
            while ( PathExtensions.GetObjectState( downloadingFileInfo.LocalFilePath ) == ObjectStateType.Locked );
        ...
        private void DeleteTempFile( DownloadingFileInfo downloadingFileInfo )
        {
